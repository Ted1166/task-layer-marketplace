...
interface AgentDetail {
  ...
  sessions: { spendCap: number; spent: number; exceededCap: boolean; actedAfterExp: boolean; revoked: boolean }[];
...
export default function AgentDetailPage({ params }: { params: { id: string } }) {
  ...
  async function submitReport() {
    ...
    await fetch("/api/reports", {
      ...
      body: JSON.stringify({ agentId: params.id, summary: reportText }),
    });
    setReportText("");
    setReportSent(true);
  }
    ...
    <div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="rounded border border-border bg-panel p-5">
          ...
          <button className="w-full rounded border border-primary py-2 text-sm hover:bg-primary/10 transition-colors">
            Hire this agent
          </button>
          <p className="mt-2 text-xs text-muted">
            Activates a scoped BSC wallet session for this agent. Stub — wire to Altana session issuance.
          </p>
        </div>
      </div>
